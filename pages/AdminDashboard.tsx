...
import { Order, UserRole, Vendor, Dish, User } from '../types';
...
type AdminTab = 'dashboard' | 'orders' | 'vendors' | 'users' | 'add-vendor';

const AdminDashboard: React.FC = () => {
    const [activeTab, setActiveTab] = useState<AdminTab>('dashboard');
    const { dataState, loading, addVendor, updateVendor, deleteVendor, addUser, updateUser, deleteUser, updateOrderStatus } = useData();
    const { vendors, users, orders } = dataState;
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="mb-8 border-b border-slate-200 dark:border-slate-700">
                <nav className="-mb-px flex space-i-6 overflow-x-auto" aria-label="Tabs">
                    ...
                    <TabButton name="Vendors" tab="vendors" activeTab={activeTab} setActiveTab={setActiveTab} />
                    <TabButton name="المستخدمين" tab="users" activeTab={activeTab} setActiveTab={setActiveTab} />
                    <TabButton name="Add Vendor" tab="add-vendor" activeTab={activeTab} setActiveTab={setActiveTab} />
                </nav>
            </div>

            <div>
                {activeTab === 'dashboard' && <DashboardContent vendors={vendors} users={users} orders={orders} />}
                {activeTab === 'orders' && <OrdersContent orders={orders} updateOrderStatus={updateOrderStatus} />}
                {activeTab === 'vendors' && <VendorsContent vendors={vendors} addVendor={addVendor} updateVendor={updateVendor} deleteVendor={deleteVendor} />}
                {activeTab === 'users' && <UsersContent users={users} vendors={vendors} addUser={addUser} updateUser={updateUser} deleteUser={deleteUser} />}
                {activeTab === 'add-vendor' && <AddVendorForm />}
            </div>
        </div>
    );
};
...
// --- Dashboard Content ---
const DashboardContent: React.FC<{ vendors: Vendor[], users: User[], orders: Order[] }> = ({ vendors, users, orders }) => {
    ...
    const todaysOrders = orders ? orders.filter(o => o.date === todayStr) : [];
    
    const deliveredTodaysOrders = todaysOrders ? todaysOrders.filter(o => o.status === 'delivered') : [];
    const todaysRevenue = deliveredTodaysOrders ? deliveredTodaysOrders.reduce((sum, order) => sum + order.totalPrice, 0) : 0;
    
    const totalCustomers = users ? users.filter(u => u.role === UserRole.CUSTOMER).length : 0;
    ...
    return (
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard title="إجمالي المطاعم" value={totalVendors} />
        </div>
    );
};
...
const OrderCard: React.FC<{ order: Order, updateOrderStatus: (id: number, status: Order['status']) => void, onViewNotes: (order: Order) => void }> = ({ order, updateOrderStatus, onViewNotes }) => (
    <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            ...
            <div className="text-slate-500 dark:text-slate-400">المتجر:</div><div className="font-medium text-slate-700 dark:text-slate-200">{order.vendorName}</div>
...
const VendorsContent: React.FC<{ 
...
}> = ({ vendors, updateVendor, deleteVendor }) => {
    ...
    const handleSaveVendor = async (data: any) => {
        try {
            ...
            alert("Failed to save vendor. Check the console for details.");
        }
    };
    ...
    if (selectedVendor) {
        return <VendorMenuManagement vendor={selectedVendor} onBack={() => setSelectedVendor(null)} />
    }

    if (editingVendor) {
        return <EditVendorForm vendor={editingVendor} onSave={() => setEditingVendor(null)} />
    }
                ...
                {vendors && vendors.map(res => (
                    <div key={res.id} className="grid grid-cols-2 sm:grid-cols-5 gap-4 items-center bg-white dark:bg-slate-800 sm:hover:bg-slate-50 dark:sm:hover:bg-slate-700/50 p-4 sm:px-6 rounded-lg shadow sm:shadow-none sm:rounded-none">
                        ...
                        <div className="col-span-2 sm:col-span-1 flex justify-start sm:justify-end items-center gap-3 mt-2 sm:mt-0">
                            <button onClick={() => setSelectedVendor(res)} className="text-primary-600 dark:text-primary-400 hover:underline text-sm font-semibold">إدارة المنيو</button>
                            <button onClick={() => openModalForEdit(res)} className="text-slate-500 hover:text-blue-600 p-1"><EditIcon className="w-5 h-5"/></button>
                            <button onClick={() => handleDeleteVendor(res.id)} className="text-slate-500 hover:text-red-600 p-1"><TrashIcon className="w-5 h-5"/></button>
                        </div>
                    </div>
                ))}
            </div>
...
const UsersContent: React.FC<{ 
...
}> = ({ users, vendors, addUser, updateUser, deleteUser }) => {
    ...
    return (
        <>
            ...
            <UserFormModal isOpen={isUserModalOpen} onClose={() => setIsUserModalOpen(false)} onSave={handleSaveUser} user={editingUser} vendors={vendors} />
        </>
    );
};
...
const UserFormModal: React.FC<{ isOpen: boolean, onClose: () => void, onSave: (data: Partial<User>) => void, user: User | null, vendors: Vendor[] }> = ({ isOpen, onClose, onSave, user, vendors }) => {
    const [formData, setFormData] = useState({ name: '', email: '', password: '', role: UserRole.CUSTOMER, vendorId: '' });
    const isEditing = !!user;

    useEffect(() => {
        if (user) {
            setFormData({ name: user.name, email: user.email, password: '', role: user.role, vendorId: user.vendorId?.toString() || '' });
        } else {
            setFormData({ name: '', email: '', password: '', role: UserRole.DELIVERY, vendorId: '' });
        }
    }, [user, isOpen]);

    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        const dataToSave: Partial<User> = { ...formData, vendorId: formData.vendorId ? parseInt(formData.vendorId) : undefined };
        if (isEditing && !formData.password) {
            delete dataToSave.password;
        }
        if (dataToSave.role !== UserRole.RESTAURANT) {
            delete dataToSave.vendorId;
        }
        onSave(dataToSave);
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'تعديل مستخدم' : 'إضافة مستخدم جديد'}>
            <form onSubmit={handleSubmit}><div className="p-6 space-y-4">
                ...
                {formData.role === UserRole.RESTAURANT && (
                    <div>
                        <label htmlFor="vendor-select" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">المتجر المسؤول عنه</label>
                        <select id="vendor-select" value={formData.vendorId} onChange={e => setFormData({ ...formData, vendorId: e.target.value })} required className="input-field">
                            <option value="" disabled>اختر متجر...</option>
                            {vendors.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                        </select>
                    </div>
                )}
            </div><div className="bg-slate-50 dark:bg-slate-700/50 px-6 py-4 flex justify-end gap-3 rounded-b-lg">
